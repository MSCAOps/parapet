...
class AutomatedOpsView(object):
    ...
    def __init__(self, db, region, account, application, phase):
        self.db = db
        self.region = region
        ...
        serverMap = monQuery.select(self.db.monitorMap.ALL, self.db.serverInfo.ALL).first() or self.defaultServerMap
    ...
    def opsviewname(self, instanceId):
        ...
        """
        return '{0}_{1}'.format(self.application or 'infra', instanceId)
    ...
    def getOpsViewObjByName_(self, objType, name):
        """\
            Return the id of the OpsView entity which matches the provided name.
